<div>
    ...
    <x-btn-admin-template
        wire:click="$dispatch('openModal', { component: 'admin.chose-event-type.create'})">Chose Event Category</x-btn-admin-template>
@endif
    <div
        class="py-1 font-extrabold text-sm text-transparent divider divider-info bg-clip-text bg-gradient-to-r from-pink-500 to-violet-500">
        {{ $divider }}</div>
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-1">
                ...
                <div class="dropdown dropdown-end ">
                    ...
                    <div tabindex="0"
                        ...
                        <div class="relative">
                            ...
                            <div class="overflow-auto scroll-smooth focus:scroll-auto h-40  mb-2" wire:target='report_byName' wire:loading.class='hidden'>
                                @forelse ($Report_By as $report_by)
                                    <div wire:click="reportedBy({{ $report_by->id }})" class="border-b border-base-200 flex flex-col cursor-pointer ">
                                        <strong class="text-[10px] text-slate-800">{{ $report_by->lookup_name }}</strong>
                                    </div>
                                @empty
                                    <strong class="text-xs bg-clip-text text-transparent bg-gradient-to-r from-rose-400 to-rose-800">Name Not Found!!!</strong>
                                @endforelse
                            </div>
                            <div class="hidden text-center pt-5"  wire:target='report_byName' wire:loading.class.remove='hidden'> <x-loading-spinner /></div>
                ...
                <div class="dropdown dropdown-end ">
                    <x-input wire:model.live='report_toName' :error="$errors->get('report_toName')" class="cursor-pointer"
                        ...
                        <div class="relative">
                            ...
                            <div class="overflow-auto scroll-smooth focus:scroll-auto h-40  mb-2" wire:target='report_toName' wire:loading.class='hidden'>
                                @forelse ($Report_To as $report_to)
                                    <div wire:click="reportedTo({{ $report_to->id }})" class="border-b border-base-200 flex flex-col cursor-pointer ">
                            ...
                            <div class="hidden text-center pt-5"  wire:target='report_toName' wire:loading.class.remove='hidden'> <x-loading-spinner /></div>
            ...
            <div class="w-full max-w-md xl:max-w-xl form-control">
                ...
                <div class="relative">
                    ...
                    <div class="avatar absolute  inset-y-0 right-0">
        ...
        <table class="table table-xs">
            @foreach ($RiskAssessment as $item)
                ...
                <tr>
                    ...
                    <td class="text-xs pl-2 border-slate-400 border-2">{{ $item->RiskAssessment->coordinator }}</td>
                </tr>
            @endforeach
        </table>
